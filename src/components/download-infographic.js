...
const getInfographicName = posterUrl => {
  try {
    const url = new URL(posterUrl, "https://www.elevatelearning.org")
    return decodeURIComponent(
      url.pathname.split("/").filter(Boolean).pop() || "Infographic poster"
    )
  } catch {
...
const DownloadInfographic = ({ previewUrl, posterUrl, embedded = false }) => {
  ...
  const infographicName = getInfographicName(posterUrl)
  const bodyColumnWidth = previewUrl ? 9 : 12

  const wrapCard = content => {
    const card = (
      <Card>
        <Row className="g-0">
          <Col md={bodyColumnWidth}>{content}</Col>
          {previewUrl ? (
            <Col md={3}>
              <Card.Img src={previewUrl} alt="Infographic preview" />
            </Col>
          ) : null}
        </Row>
      </Card>
    ...
    if (embedded) {
      return <div className="infographic-wrapper">{card}</div>
    }

    return (
      <Container className="infographic-wrapper">
  ...
  const stepOne = () => {
    return wrapCard(
      <Card.Body>
        ...
        <Card.Text as="div">
          <p>
            Fill out your information below to download our free infographic for
            this article now.
          </p>
          <Form
            ref={formRef}
    ...
    return wrapCard(
      <Card.Body>
        ...
        <Card.Text as="div" className="text-center my-8">
          <p>
            Thank you for your interest in this infographic and please use the
            link below to download a copy for your personal use.
          </p>
          <p>
  ...
  const displayFormStep = () => {
    switch (formState.step) {
      case 0:
        return posterUrl ? stepOne() : null
      case 1:
        return posterUrl ? stepTwo() : null
      default:
        return posterUrl ? stepOne() : null
    }
  }
